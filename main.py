...
def verifyCheckCode(driver):
    try:
        ...
        for x in range(len(cf.items("check_code"))):
            print("check_code : " + cf.items("check_code")[x][1])
            ...
            WebDriverWait(driver, 1, 0.2).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, '#checkCode')))
            driver.find_element(By.CSS_SELECTOR, '#checkCode').send_keys(cf.items("check_code")[x][1]) 
...
WebDriverWait(driver, 10, 0.2).until(EC.visibility_of_element_located(
...
driver.find_element(
    By.XPATH, '//*[@id="email"]').send_keys(cf.get("account_info", "username"))
WebDriverWait(driver, 10, 0.2).until(
    EC.visibility_of_element_located((By.XPATH, '//*[@id="pass"]')))
driver.find_element(
    By.XPATH, '//*[@id="pass"]').send_keys(cf.get("account_info", "password"))
WebDriverWait(driver, 10, 0.2).until(
...
while True:
    ...
    try:
        WebDriverWait(driver, 120, 0.2).until(
            ...
            if y == 0:
                ...
                for x in range(len(ticketList)):
                    ...
                    if "熱賣中" in ticketList[x].text:
                        ...
                        if int(cf.get("ticket_info", "amount")) <= int(splitedTicketName[remainAmount-1]):
                            ticketList[x].click()
                            exit_flag = True
                ...
                print("y=1")
                WebDriverWait(driver, 10, 0.2).until(EC.presence_of_all_elements_located(
                ...
                for x in range(len(ticketList)):
                    ...
                    if "熱賣中" in ticketList[x].text:
                        ...
                        print("condition "+str(y)+" : " +
                              ticketList[x].text + "門票剩餘"+splitedTicketName[remainAmount-1])
    ...
    while True:    
        ...
        Select(driver.find_element(By.CSS_SELECTOR, '.normal tr select')
                    ).select_by_value(cf.get("ticket_info", "amount"))
