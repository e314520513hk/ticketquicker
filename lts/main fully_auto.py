...
from ticketholder import captcha
import json
import time
import requests
import urllib
        ...
        try:
            ...
            while True:
                WebDriverWait(driver, 2, 0.1).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, 'div.zone.area-list > ul > li')))
            ...
            print("錯誤類型: "+ type(e).__name__+" 錯誤原因:"+str(e))
            if type(e).__name__ == 'TimeoutException' and '票種' in driver.title:
                print('不用選座位，直接進入票種')
    ...
    if redirectFlag and ('票種' in driver.title) or not redirectFlag:
        ...
        while True:    
            try:
                ...
                ele_check_code = driver.find_element(By.ID, 'TicketForm_verifyCode').click()
            
                endTime = time.time()
                print('程序执行时间: ',endTime - startTime)
                
                while True:
                    ele_image = driver.find_element(By.XPATH, '//*[@id="yw0"]')
                    ele_image.screenshot("ele_image.png")
                    captcha_response = captcha.solve("ele_image.png")
                    if captcha_response:
                        driver.find_element(By.ID, 'TicketForm_verifyCode').send_keys(captcha_response['code'])
                        driver.find_element(By.ID, 'ticketPriceSubmit').click()
                        
                        print("code:"+captcha_response['code'])
                        sleep(10)
                        
                        WebDriverWait(driver, 10, 0.1).until(
                            EC.visibility_of_element_located((By.CSS_SELECTOR,'input[name="PaymentForm[payment_id]"]')))
                        # driver.find_element(By.CSS_SELECTOR,'input[name="PaymentForm[payment_id]"]').click()
                        for ele in driver.find_elements(By.CSS_SELECTOR,'input[name="PaymentForm[payment_id]"]'):
                            ele.click()
                        sleep(1)
                        WebDriverWait(driver, 10, 0.1).until(
                        ...
                        sleep(1)
                        WebDriverWait(driver, 10, 0.1).until(
                            EC.visibility_of_element_located((By.CSS_SELECTOR, '.pay-final')))    
                        
                        print(driver.find_element(By.CSS_SELECTOR, '.pay-final').text)
                        sleep(20)
